...
struct ScorePosesKernel1D
{
        ...
        std::uint32_t nposes) const
    {
        ...
        std::uint64_t const workStride
            = static_cast<std::uint64_t>(linearGridSize) * static_cast<std::uint64_t>(T_PPWI);

        for(std::uint64_t base = static_cast<std::uint64_t>(linearGridThreadIndex) * T_PPWI; base < nposes;
            base += workStride)
        {
#pragma unroll
            for(std::uint32_t j = 0u; j < T_PPWI; ++j)
            {
                ...
                float const t00 = cy * cz;
                float const t01 = sx * sy * cz - cx * sz;
                float const t02 = cx * sy * cz + sx * sz;
                float const t03 = tx[i];
                float const t10 = cy * sz;
                float const t11 = sx * sy * sz + cx * cz;
                float const t12 = cx * sy * sz - sx * cz;
                float const t13 = ty[i];
                float const t20 = -sy;
                float const t21 = sx * cy;
                float const t22 = cx * cy;
                float const t23 = tz[i];
...
#    pragma unroll
                    ...
                    for(std::uint32_t ip = 0; ip < natpro; ++ip)
                    {
                        ...
                        float chrg_e = chrg_init
                                       * ((zone1 ? ONE : (ONE - distbb * elcdst1)) * ((distbb < elcdst) ? ONE : ZERO));
                        if(type_E)
    ...
    ThreadSpecInfo makeThreadSpec(std::uint32_t requestedWgsize) const
    {
        ...
        else
        {
            ...
            std::uint32_t numBlocks
                = static_cast<std::uint32_t>((static_cast<std::uint64_t>(nposes) + workPerBlock - 1u) / workPerBlock);
            if(numBlocks == 0u)
                numBlocks = 1u;
